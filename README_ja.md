...
libpino は、ピュア C99 で書かれた軽量シリアライゼーションライブラリです。ハンドラーベースのアーキテクチャにより、構造化データのパックとアンパックを行い、バイトオーダー変換用の補助関数と任意の SIMD 実装を利用できます。

## 特徴

- **純粋な C99 実装** - 実行時の外部依存なし
- **ハンドラーベースアーキテクチャ** - カスタム型ハンドラーによる拡張可能な設計
- **バイトオーダー変換ヘルパー** - リトルエンディアン、ビッグエンディアン、ネイティブバイトオーダー間の変換補助を提供
- **任意の SIMD 経路** - 対応環境では AVX2 (amd64)、NEON (arm64)、WASM SIMD128 実装を利用可能
- **カスタムメモリマネージャー** - ハンドラー内の確保処理はライブラリのメモリマネージャーを経由
- **WebAssembly 対応** - Emscripten を使用して WASM にコンパイル可能
- **テスト整備** - 単体テスト、サニタイザ、Valgrind 向けの構成を含む
...
PINO ライブラリが保持しているグローバルなハンドラー登録状態を解放します。既存の PINO オブジェクトは、それぞれに対して `pino_destroy()` を呼ぶまで有効です。
...
ライフサイクル上の注意: `pino_free()` はグローバルな登録状態だけを解放します。各 `pino_t` は引き続き `pino_destroy()` で個別に解放する必要があります。

データ表現に関する注意: `pino_pack()` と `pino_unpack()` は、プログラム内で使うネイティブなメモリ表現をそのまま扱います。一方、`pino_serialize()` と `pino_unserialize()` は、little-endian のバイト列との相互変換を担当します。
...
`elem_size` には変換単位の要素サイズを渡します。スカラ値なら `sizeof(型)`、配列なら `sizeof(array[0])` を使ってください。
...
libpino は軽量ながらも次のようなユースケースを目的とする場合に適しています：

- **クロスプラットフォームのバイナリデータ交換** - 固定の wire format とバイトオーダー変換ヘルパーを組み合わせたい場合
- **カスタムシリアライゼーション形式** - ハンドラーベースアーキテクチャによりドメイン固有の形式を定義したい場合
- **メモリコピー主体の処理** - 対応ターゲットで SIMD 実装を利用したい場合
- **組み込みシステム** - 実行時の外部依存を避けたい場合
- **WebAssembly アプリケーション** - Emscripten ビルドを利用したい場合
