...
def extract_author(comment):
    match = re.search(r"^\s*\*\s*Author\s*:\s*(.*?)\s*$", comment, re.MULTILINE)
    if not match:
        return None
...
def insert_header(content, filename, author=None):
    author_line = f" * Author: {author}\n" if author else ""
    header = LICENSE_HEADER.format(filename=filename, author_line=author_line)
...
def process_file(file_path):
    ...
    try:
        ...
        content = insert_header(content, file_path.name, author)
