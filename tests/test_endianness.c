...
void test_simd_memcpy_large_array_64bit(void)
{
    ...
    for (i = 0; i < num_elements; i++) {
        if (dest[i] != expected[i]) {
            snprintf(msg, sizeof(msg), "Element %zu: expected 0x%016llX, got 0x%016llX", i,
...
void test_simd_byteswap_pattern(void)
{
    ...
    for (i = 0; i < num_elements; i++) {
        ...
        if (is_little_endian()) {
            ...
            if (dest_bytes[0] != b3 || dest_bytes[1] != b2 || dest_bytes[2] != b1 || dest_bytes[3] != b0) {
                snprintf(msg, sizeof(msg),
                         "Element %zu byte order wrong: got [%02X,%02X,%02X,%02X], "
                         "expected [%02X,%02X,%02X,%02X]",
                         i, dest_bytes[0], dest_bytes[1], dest_bytes[2], dest_bytes[3], b3, b2, b1, b0);
                TEST_FAIL_MESSAGE(msg);
            }
        } else {
            if (dest[i] != src[i]) {
