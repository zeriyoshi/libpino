...
#define PH_NAME_STATIC_FIELDS_STRUCT(name)                                     \
  _ph_handler_##name##_static_fields_struct
...
#define PH_SIGNATURE_SERIALIZE_SIZE                                            \
  (const void *PH_ARG_THIS, const void *PH_ARG_STATIC_FIELDS)
#define PH_SIGNATURE_SERIALIZE                                                 \
  (const void *PH_ARG_THIS, const void *PH_ARG_STATIC_FIELDS, void *PH_ARG_DST)
#define PH_SIGNATURE_UNSERIALIZE                                               \
  (void *PH_ARG_THIS, void *PH_ARG_STATIC_FIELDS, const void *PH_ARG_SRC,      \
   size_t PH_ARG_SRC_SIZE)
#define PH_SIGNATURE_PACK                                                      \
  (void *PH_ARG_THIS, void *PH_ARG_STATIC_FIELDS, const void *PH_ARG_SRC,      \
   size_t PH_ARG_SIZE)
#define PH_SIGNATURE_UNPACK_SIZE                                               \
  (const void *PH_ARG_THIS, const void *PH_ARG_STATIC_FIELDS)
#define PH_SIGNATURE_UNPACK                                                    \
  (const void *PH_ARG_THIS, const void *PH_ARG_STATIC_FIELDS, void *PH_ARG_DST)
...
#define PH_DEF_STRUCT_END                                                      \
  ;                                                                            \
  __pragma(pack(pop))
#define PH_DEF_STATIC_FIELDS_STRUCT(name)                                      \
  __pragma(pack(push, 1)) struct PH_NAME_STATIC_FIELDS_STRUCT(name)
...
#define PH_DEF_STRUCT_END ;
#define PH_DEF_STATIC_FIELDS_STRUCT(name)                                      \
  struct __attribute__((packed)) PH_NAME_STATIC_FIELDS_STRUCT(name)
#define PH_DEF_STATIC_FIELDS_STRUCT_END ;
#else
#define PH_DEF_STRUCT(name) struct PH_NAME_STRUCT(name)
#define PH_DEF_STRUCT_END ;
#define PH_DEF_STATIC_FIELDS_STRUCT(name)                                      \
  struct PH_NAME_STATIC_FIELDS_STRUCT(name)
#define PH_DEF_STATIC_FIELDS_STRUCT_END ;
#warning "Unknown compiler, struct packing may not work correctly"
#endif

#define PH_DEFUN_SERIALIZE_SIZE(name)                                          \
  static size_t PH_NAME_FUNC_SERIALIZE_SIZE(name) PH_SIGNATURE_SERIALIZE_SIZE
#define PH_DEFUN_SERIALIZE(name)                                               \
  static bool PH_NAME_FUNC_SERIALIZE(name) PH_SIGNATURE_SERIALIZE
#define PH_DEFUN_UNSERIALIZE(name)                                             \
  static bool PH_NAME_FUNC_UNSERIALIZE(name) PH_SIGNATURE_UNSERIALIZE
#define PH_DEFUN_PACK(name)                                                    \
  static bool PH_NAME_FUNC_PACK(name) PH_SIGNATURE_PACK
#define PH_DEFUN_UNPACK_SIZE(name)                                             \
  static size_t PH_NAME_FUNC_UNPACK_SIZE(name) PH_SIGNATURE_UNPACK_SIZE
#define PH_DEFUN_UNPACK(name)                                                  \
  static bool PH_NAME_FUNC_UNPACK(name) PH_SIGNATURE_UNPACK
#define PH_DEFUN_CREATE(name)                                                  \
  static void *PH_NAME_FUNC_CREATE(name) PH_SIGNATURE_CREATE
#define PH_DEFUN_DESTROY(name)                                                 \
  static void PH_NAME_FUNC_DESTROY(name) PH_SIGNATURE_DESTROY

#define PH_THIS_P(name, ptr) ((struct PH_NAME_STRUCT(name) *)ptr)
#define PH_THIS_STATIC_P(name, ptr)                                            \
  ((struct PH_NAME_STATIC_FIELDS_STRUCT(name) *)ptr)
#define PH_THIS_STATIC_GET_P(name, ptr, param, dest)                           \
  do {                                                                         \
    PH_MEMCPY_L2N(dest, &PH_THIS_STATIC_P(name, ptr)->param,                   \
                  sizeof(PH_THIS_STATIC_P(name, ptr)->param));                 \
  } while (0)
#define PH_THIS_STATIC_SET_P(name, ptr, param, src)                            \
  do {                                                                         \
    PH_MEMCPY_N2L(&PH_THIS_STATIC_P(name, ptr)->param, src,                    \
                  sizeof(PH_THIS_STATIC_P(name, ptr)->param));                 \
  } while (0)
#define PH_THIS(name) (PH_THIS_P(name, PH_ARG_THIS))
#define PH_THIS_STATIC(name) (PH_THIS_STATIC_P(name, PH_ARG_STATIC_FIELDS))
#define PH_THIS_STATIC_GET(name, param, dest)                                  \
  PH_THIS_STATIC_GET_P(name, PH_ARG_STATIC_FIELDS, param, dest)
#define PH_THIS_STATIC_SET(name, param, src)                                   \
  PH_THIS_STATIC_SET_P(name, PH_ARG_STATIC_FIELDS, param, src)
#define PH_PINO_P(name, pino) (PH_THIS_P(name, ((pino_t *)pino)->this))
#define PH_PINO_STATIC_P(name, pino)                                           \
  ((struct PH_NAME_STATIC_FIELDS_STRUCT(name) *)pino->static_fields)
#define PH_PINO_STATIC_GET_P(name, pino, param, dest)                          \
  PH_THIS_STATIC_GET_P(name, PH_PINO_STATIC_P(name, pino), param, dest)
#define PH_PINO_STATIC_SET_P(name, pino, param, src)                           \
  PH_THIS_STATIC_SET_P(name, PH_PINO_STATIC_P(name, pino), param, src)
...
#define PH_MALLOC(name, size)                                                  \
  pino_memory_manager_malloc(PH_NAME_HANDLER(name).entry, size)
#define PH_CALLOC(name, count, size)                                           \
  pino_memory_manager_calloc(PH_NAME_HANDLER(name).entry, count, size)
#define PH_FREE(name, ptr)                                                     \
  pino_memory_manager_free(PH_NAME_HANDLER(name).entry, ptr)

#define PH_MEMCPY(dst, src, size) memcpy(dst, src, size)
#define PH_MEMCPY_N2L(dst, src, size)                                          \
  pino_endianness_memcpy_native2le(dst, src, size)
#define PH_MEMCPY_N2B(dst, src, size)                                          \
  pino_endianness_memcpy_native2be(dst, src, size)
#define PH_MEMCPY_L2N(dst, src, size)                                          \
  pino_endianness_memcpy_le2native(dst, src, size)
#define PH_MEMCPY_B2N(dst, src, size)                                          \
  pino_endianness_memcpy_be2native(dst, src, size)
...
#define PH_SERIALIZE_DATA(name, src, size)                                     \
  do {                                                                         \
    PH_MEMCPY_N2L(PH_ARG_DST, PH_THIS(name)->src, size);                       \
  } while (0)
#define PH_UNSERIALIZE_DATA(name, dest, size)                                  \
  do {                                                                         \
...
#define PH_PACK_DATA(name, param, size)                                        \
  do {                                                                         \
    PH_MEMCPY_N2L(PH_THIS(name)->param, PH_ARG_SRC, size);                     \
  } while (0)
#define PH_UNPACK_DATA(name, param, size)                                      \
  do {                                                                         \
