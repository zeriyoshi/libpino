...
extern void *pino_memory_manager_malloc(/* handler_entry_t */ void *entry,
                                        size_t size) {
  size_t i;
  ...
  if (((handler_entry_t *)entry)->mm.usage >=
...
extern void *pino_memory_manager_calloc(/* handler_entry_t */ void *entry,
...
extern void pino_memory_manager_free(/* handler_entry_t */ void *entry,
                                     void *ptr) {
  size_t i;
  ...
  for (i = 0; i < ((handler_entry_t *)entry)->mm.capacity; i++) {
    if (((handler_entry_t *)entry)->mm.ptrs[i] &&
        ((handler_entry_t *)entry)->mm.ptrs[i] == ptr) {
      pfree(ptr);
