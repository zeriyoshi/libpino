...
static inline pino_t *pino_create(pino_magic_safe_t magic,
                                  pino_handler_t *handler, size_t size) {
  pino_t *pino;
...
extern bool pino_init(void) { return pino_handler_init(HANDLER_STEP); }

extern void pino_free(void) { pino_handler_free(); }
...
extern size_t pino_serialize_size(const pino_t *pino) {
  ...
  if (handler_size > SIZE_MAX - sizeof(pino_magic_t) -
                         sizeof(pino_static_fields_size_t) -
                         pino->static_fields_size) {
    return 0;
  }

  total_size = handler_size + sizeof(pino_magic_t) +
               sizeof(pino_static_fields_size_t) + pino->static_fields_size;
...
extern bool pino_serialize(const pino_t *pino, void *dest) {
  ...
  pmemcpy_n2l(((char *)dest) + sizeof(pino_magic_t), &pino->static_fields_size,
              sizeof(pino_static_fields_size_t));

  /* fields always use LE */
  pmemcpy(((char *)dest) + sizeof(pino_magic_t) +
              sizeof(pino_static_fields_size_t),
          pino->static_fields, pino->static_fields_size);

  return pino->handler->serialize(pino->this, pino->static_fields,
                                  ((char *)dest) + sizeof(pino_magic_t) +
                                      sizeof(pino_static_fields_size_t) +
...
extern pino_t *pino_unserialize(const void *src, size_t size) {
  ...
  pmemcpy_l2n(&fields_size, ((char *)src) + sizeof(pino_magic_t),
              sizeof(pino_static_fields_size_t));

  if (size <
      sizeof(pino_magic_t) + sizeof(pino_static_fields_size_t) + fields_size) {
    return NULL;
  }
  ...
  pino = pino_create(magic, handler,
                     size - sizeof(pino_magic_t) -
                         sizeof(pino_static_fields_size_t) - fields_size);
  if (!pino) {
    return NULL;
  }

  /* always LE */
  pmemcpy(pino->static_fields,
          ((char *)src) + sizeof(pino_magic_t) +
              sizeof(pino_static_fields_size_t),
          fields_size);
...
extern pino_t *pino_pack(pino_magic_safe_t magic, const void *src,
                         size_t size) {
  pino_t *pino;
  pino_handler_t *handler;
...
extern uint32_t pino_version_id() { return (uint32_t)PINO_VERSION_ID; }
