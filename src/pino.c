...
{
    if (!pino || !pino->handler || !pino->this || !pino->static_fields) {
        return 0;
    }
...
{
    if (!pino || !dest || !pino->handler || !pino->this || !pino->static_fields) {
        return false;
    }
...
extern pino_t *pino_unserialize(const void *src, size_t size)
{
    ...
    /* Validate that fields_size matches the handler's expected size */
